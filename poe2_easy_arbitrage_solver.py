...
with top1:
    start_currency = st.selectbox("Start / End Currency", CURRENCIES, index=0)
with top2:
    bankroll = st.number_input(f"Current {start_currency}", min_value=0.0, value=548.0, step=1.0)
with top3:
    max_trades = st.slider("Max trades", 2, 5, 5)
...
with tab2:
    ...
    if st.button("Transfer paste to manual table"):
        ...
        if not rows:
            ...
            st.success(f"Updated {updated} row(s). Open Manual Table to review.")
            st.rerun()
