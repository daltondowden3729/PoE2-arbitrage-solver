...
def load_saved_data():
    if SAVE_FILE.exists():
        try:
            ...
            df = pd.DataFrame(data.get("rates", DEFAULT_ROWS), columns=["Currency", "Trade To", "Buy", "Sell"])
            bankroll = float(data.get("bankroll", 548.0))
            max_trades = int(data.get("max_trades", 5))
            return df, bankroll, max_trades
        except Exception:
            pass
    return make_default_df(), 548.0, 5

def save_data(df, bankroll, max_trades):
    data = {
        "rates": df[["Currency", "Trade To", "Buy", "Sell"]].values.tolist(),
...
def evaluate_path(path, bankroll, edges):
    ...
    for frm, to in zip(path, path[1:]):
        ...
        steps.append({"text": text, "reason": reason, "ratio": ratio, "math": math})
...
def show_results(results, bankroll):
    ...
    for rank, r in enumerate(shown, 1):
        ...
        with st.expander(f"{icon} #{rank} | Profit: {fmt(r['profit'])} Ex | ROI: {r['roi']:.2f}% | Final: {fmt(r['final'])} Ex", expanded=(rank == 1)):
...
st.write("This version saves your numbers, so they do not reset when the app refreshes.")

if "rates_df" not in st.session_state:
    df, saved_bankroll, saved_max_trades = load_saved_data()
    st.session_state.rates_df = df
    st.session_state.saved_bankroll = saved_bankroll
    st.session_state.saved_max_trades = saved_max_trades
    st.session_state.last_import_changes = []

bankroll = st.number_input("Current Exalted", min_value=0.0, value=float(st.session_state.saved_bankroll), step=1.0)
max_trades = st.slider("Max number of trades to check", min_value=2, max_value=5, value=int(st.session_state.saved_max_trades))

st.subheader("1) Update Rates")
tab_manual, tab_paste = st.tabs(["Manual Table", "Paste Table"])

with tab_manual:
    st.caption("This is the master table. Your edits are saved automatically.")
    c1, c2 = st.columns([1, 1])
    with c1:
        if st.button("Save current table"):
            save_data(st.session_state.rates_df, bankroll, max_trades)
            st.success("Saved.")
    with c2:
    ...
    st.session_state.saved_bankroll = bankroll
    st.session_state.saved_max_trades = max_trades
    save_data(st.session_state.rates_df, bankroll, max_trades)

    if st.session_state.last_import_changes:
        with st.expander("Last paste import changes", expanded=False):
            change_df = pd.DataFrame(st.session_state.last_import_changes, columns=["Currency", "Trade To", "Old Buy", "New Buy", "Old Sell", "New Sell"])
            st.dataframe(change_df, use_container_width=True, hide_index=True)

with tab_paste:
    st.caption("Paste your copied table here. Then transfer it into the Manual Table. It will also save automatically.")
    ...
    if st.button("Transfer Paste to Manual Table"):
        ...
        if not rows:
            ...
            save_data(st.session_state.rates_df, bankroll, max_trades)
            st.success(f"Transfer complete and saved: {updated} row(s) updated, {added} row(s) added, {skipped} row(s) skipped.")
            st.info("Open the Manual Table tab to review the updated values.")
