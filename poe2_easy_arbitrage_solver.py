...
def default_df():
    ...
    for base in CURRENCIES:
        for target in CURRENCIES:
            ...
            # Default Buy/Sell are optional helper values.
            # If left as 1 and 1, Amount remains whatever is entered.
            rows.append([base, target, 1.0, 1.0, amount])
...
def parse_paste(text):
    ...
    for raw in text.splitlines():
        ...
        if len(parts) >= 3 and parts[0] in CURRENCIES and parts[1] in CURRENCIES:
            try:
                rows.append([parts[0], parts[1], 1.0, 1.0, float(parts[2])])
            except Exception:
                pass
            continue
        if current and len(parts) >= 2:
            try:
                ...
                if target in CURRENCIES and target != current:
                    rows.append([current, target, 1.0, 1.0, val])
            except Exception:
                pass
    return rows
...
with tab1:
    ...
    with c2:
        if st.button("Reset values"):
            ...
            st.rerun()
    with c3:
        if st.button("Apply Buy ÷ Sell to Amount"):
            st.session_state.rates = apply_buy_sell_to_amount(st.session_state.rates)
            save_state(st.session_state.rates, start_currency, bankroll, max_trades)
            st.success("Amount updated from Buy ÷ Sell.")
            st.rerun()
    ...
    st.session_state.rates = clean_rates_df(edited)
    save_state(st.session_state.rates, start_currency, bankroll, max_trades)
