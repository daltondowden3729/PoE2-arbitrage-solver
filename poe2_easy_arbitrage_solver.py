...
def default_df():
    ...
    for base in CURRENCIES:
        for target in CURRENCIES:
            if base == target:
                continue
            amount = DEFAULT_RATES.get(base, {}).get(target, 0.0)
            # Buy and Sell are set so Buy ÷ Sell equals Amount.
            # This makes the helper match the Amount column immediately.
            rows.append([base, target, amount, 1.0, amount])
    return pd.DataFrame(rows, columns=["Using 1", "You can buy", "Buy", "Sell", "Amount"])

def clean_rates_df(df):
    out = df.copy()

    # Old save files may only have 3 columns.
    if "Buy" not in out.columns:
        out["Buy"] = out["Amount"] if "Amount" in out.columns else 0.0
    if "Sell" not in out.columns:
        out["Sell"] = 1.0

    needed = ["Using 1", "You can buy", "Buy", "Sell", "Amount"]
    out = out[needed].copy()
...
def add_calculated_columns(df):
    out = clean_rates_df(df)

    def calc_buy_sell(row):
        try:
            buy = float(row["Buy"])
            sell = float(row["Sell"])
            if sell <= 0:
    ...
    out["Buy ÷ Sell"] = out.apply(calc_buy_sell, axis=1)

    # This keeps the old Amount field editable while also showing Buy ÷ Sell.
    # If you want Amount to equal Buy ÷ Sell, click the Apply button below.
...
def apply_buy_sell_to_amount(df):
    out = clean_rates_df(df)
    for idx, row in out.iterrows():
        try:
            ...
            sell = float(row["Sell"])
            if sell > 0:
                out.at[idx, "Amount"] = buy / sell
        except Exception:
            pass
    return out

def save_state(rates_df, start_currency, bankroll, max_trades):
    rates_only = clean_rates_df(rates_df)
    data = {
        "rates": rates_only[["Using 1", "You can buy", "Buy", "Sell", "Amount"]].values.tolist(),
...
def load_state():
    if SAVE_FILE.exists():
        try:
            ...
            if rates_raw:
                # Support older 3-column saves and newer 5-column saves.
                if len(rates_raw[0]) == 3:
                    rates = pd.DataFrame(rates_raw, columns=["Using 1", "You can buy", "Amount"])
                    rates["Buy"] = pd.to_numeric(rates["Amount"], errors="coerce").fillna(0.0)
                    rates["Sell"] = 1.0
                    rates = rates[["Using 1", "You can buy", "Buy", "Sell", "Amount"]]
                else:
                    rates = pd.DataFrame(rates_raw, columns=["Using 1", "You can buy", "Buy", "Sell", "Amount"])
                return (
                    clean_rates_df(rates),
...
def parse_paste(text):
    ...
    for raw in text.splitlines():
        ...
        if len(parts) >= 3 and parts[0] in CURRENCIES and parts[1] in CURRENCIES:
            try:
                rows.append([parts[0], parts[1], float(parts[2]), 1.0, float(parts[2])])
            except Exception:
                pass
            continue
        if current and len(parts) >= 2:
            try:
                ...
                if target in CURRENCIES and target != current:
                    rows.append([current, target, val, 1.0, val])
            except Exception:
                pass
    return rows
...
def normalize_imported_df(uploaded_df):
    ...
    if {"Using 1", "You can buy", "Amount"}.issubset(set(df.columns)):
        ...
        if "Sell" not in out.columns:
            out["Sell"] = 1.0
        out = out[["Using 1", "You can buy", "Buy", "Sell", "Amount"]].copy()
        ...
        out["Buy"] = out["Buy"].fillna(out["Amount"])
        out["Sell"] = out["Sell"].fillna(1.0)
        # If Buy/Sell are present, automatically calculate Amount from them.
        out["Amount"] = out.apply(lambda r: (r["Buy"] / r["Sell"]) if r["Sell"] and r["Sell"] > 0 else r["Amount"], axis=1)
        out = out[out["Amount"].notna() & (out["Amount"] > 0)]
        return out.reset_index(drop=True)

    # Old Currency / Trade To / Buy / Sell format
    if {"Currency", "Trade To", "Buy", "Sell"}.issubset(set(df.columns)):
        ...
        for _, r in df.iterrows():
            ...
            if currency not in CURRENCIES or trade_to not in CURRENCIES or currency == trade_to:
                continue
            if buy > 0 and sell > 0:
                rows.append([trade_to, currency, buy, sell, sell / buy])
        return pd.DataFrame(rows, columns=["Using 1", "You can buy", "Buy", "Sell", "Amount"])
...
def transfer_to_manual(current_df, imported_df):
    df = clean_rates_df(current_df)
    imported = clean_rates_df(imported_df)
    updated = 0
    added = 0
    skipped = 0
    changes = []
    for _, row in imported.iterrows():
        ...
        if mask.any():
            ...
            df.loc[len(df)] = [base, target, buy, sell, amount]
            added += 1
            changes.append([base, target, None, amount])
    return df, updated, added, skipped, changes

st.title("PoE2 Easy Arbitrage Solver")
st.caption("Rule: **Amount** means using 1 currency, you can buy X of another currency. Buy ÷ Sell helps calculate that Amount.")
...
with tab1:
    ...
    with c2:
        if st.button("Reset values"):
            ...
            st.rerun()
    with c3:
        st.caption("Buy ÷ Sell now updates Amount automatically.")
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "Sell": st.column_config.NumberColumn("Sell", min_value=0.0, step=0.0001, format="%.4f"),
            "Buy ÷ Sell": st.column_config.NumberColumn("Buy ÷ Sell", disabled=True, format="%.4f"),
    ...
    # Automatically use Buy ÷ Sell as the Amount value and save it.
    st.session_state.rates = apply_buy_sell_to_amount(clean_rates_df(edited))
    save_state(st.session_state.rates, start_currency, bankroll, max_trades)
...
with tab2:
    ...
    if st.button("Transfer paste to manual table"):
        ...
        if not rows:
            ...
            df = clean_rates_df(st.session_state.rates)
            updated = 0
            for base, target, buy, sell, amount in rows:
                mask = (df["Using 1"] == base) & (df["You can buy"] == target)
                if mask.any():
                    df.loc[mask, "Amount"] = amount
                    updated += 1
...
with tab3:
    ...
    csv_export = clean_rates_df(st.session_state.rates).to_csv(index=False).encode("utf-8")
    st.download_button(
...
for base in CURRENCIES:
    with st.expander(base, expanded=(base == start_currency)):
        ...
        sub = clean_rates_df(st.session_state.rates)
