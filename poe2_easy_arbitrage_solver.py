...
def normalize_imported_df(uploaded_df):
    """
    Accepts either:
    1) Using 1, You can buy, Amount
    2) Currency, Trade To, Buy, Sell

    Returns dataframe with columns:
    Using 1, You can buy, Amount
    """
    df = uploaded_df.copy()
    df.columns = [str(c).strip() for c in df.columns]

    # Format 1: already the preferred format
    ...
    # Format 2: Currency / Trade To / Buy / Sell
    if {"Currency", "Trade To", "Buy", "Sell"}.issubset(set(df.columns)):
        ...
        for _, r in df.iterrows():
            ...
            # Convert into "Using 1 Trade To, you can buy X Currency"
            if buy > 0 and sell > 0:
...
def transfer_import_to_manual(current_df, imported_df):
    df = current_df.copy()
    updated = 0
    added = 0
    skipped = 0
...
with tab3:
    ...
    if st.session_state.imported_csv is not None:
        ...
        st.dataframe(st.session_state.imported_csv, use_container_width=True, hide_index=True)

        st.markdown("#### Imported Ratio Calculated")
        st.dataframe(ratio_df(st.session_state.imported_csv), use_container_width=True, hide_index=True)

        if st.button("Transfer Imported CSV to Manual Table"):
            new_df, updated, added, skipped, changes = transfer_import_to_manual(
                st.session_state.rates,
