...
def add_calculated_columns(df):
    out = clean_df(df)
    out["Amount"] = out.apply(lambda r: amount_from_buy_sell(r["Buy"], r["Sell"]), axis=1)
...
st.caption("Amount is calculated automatically: **Amount = Buy ÷ Sell**. The far-right column shows the ratio.")
...
with tab1:
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "Amount": st.column_config.NumberColumn("Amount = Buy ÷ Sell", disabled=True, format="%.4f"),
            "Ratio Calculated": st.column_config.TextColumn("Ratio Calculated", disabled=True),
        },
    )

    # Only save the editable source columns. Amount and ratio are always recalculated.
