...
def parse_pasted_rows(text):
    """
    Parses pasted text and returns only rows found in the paste:
...
def import_rows_to_manual(current_df, pasted_rows):
    """
    Updates the manual table from pasted rows.
    Matching rows are overwritten.
    New valid rows are added.
    Rows not included in paste are left alone.
    """
    df = current_df.copy()
    updated = 0
    added = 0
...
def build_edges(df):
    ...
    Each row creates ONLY ONE direction:
      Trade To -> Currency
    """
    edges = {}
...
st.write("Paste rates into the Paste Table tab, then click **Transfer Paste to Manual Table**. The Manual Table becomes the master table.")
...
with tab_manual:
    st.caption("This is the master table. Paste imports update this table, and the solver uses these values.")
...
with tab_paste:
    st.caption("Paste your copied table here. Then click the button to transfer it into the Manual Table.")
